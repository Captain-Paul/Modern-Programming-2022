...
class Target(metaclass=abc.ABCMeta):  #适配器抽象类
	@abc.abstractmethod
	def execute(self):
		pass
...
class Adapter:
	...
	def __str__(self):  #使print对象时输出对象的obj属性而不是内存地址
						#__str__方法只更改print的结果
		return str(self.obj)

	def __repr__(self): #__repr__方法更改print和命令行直接输出的结果
		return str(self.obj)
